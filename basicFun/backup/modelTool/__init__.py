...
class modelTool:
    ...
    def nms_exclusive_boxes(self,boxes,classes,exclusiveGroups,thresh):
        def match_boxes(preBoxes,preClasses):
        ...
        try:
            for i, c in enumerate(classes):
                for exclusiveGroup in exclusiveGroups:
                    if self.labelmap[c] in exclusiveGroup:
                        shareThresh=thresh/len(exclusiveGroup)
            ...
            pass
        postBoxes,postClasses=match_boxes(preBoxes,preClasses)
        return postBoxes,postClasses
    ...
    def getInfoByModel(self,image,thresh=0.7):
        ...
        boxes,classes=self.nms_exclusive_boxes(boxes,classes,exclusiveGroups,thresh)
        try:
